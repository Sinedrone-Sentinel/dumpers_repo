...
export default function StockCardListingShortcut({
  ...
  const finish = async () => {
    ...
    const base =
      result.action === 'set' ? `${label} updated` : `Added to your ${label} listing`
  ...
  const openType = (type: StockCardListingType) => {
    setFeedback(null)
    onOpen(type)
  }
  ...
  const editor = active ? (
    <div className="flex items-center gap-1.5 flex-wrap min-w-0">
      <ResourceQuantityInput
        ...
        className="site-input w-24 px-2 py-1 text-xs tabular-nums"
    ...
    <div className="mt-1.5 space-y-1.5">
      <div className="flex flex-wrap items-center gap-1.5 min-w-0">
        <button
          ...
          disabled={gated || busy}
          title={gate}
          onClick={() => openType('wts')}
          className={
            'px-2 py-1 text-xs site-btn-accent site-btn-shimmer' +
            (active === 'wts' ? ' ring-1 ring-amber-400/70' : '')
          }
        >
          WTS
        </button>
        {active === 'wts' ? editor : null}
        <button
          type="button"
          disabled={gated || busy}
          title={gate}
          onClick={() => openType('wtb')}
          className={
            'px-2 py-1 text-xs site-btn-danger site-btn-shimmer' +
            (active === 'wtb' ? ' ring-1 ring-rose-400/70' : '')
          }
        >
          WTB
        </button>
        {active === 'wtb' ? editor : null}
      </div>
      {feedback ? (
