...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Windows:** install **BP Dumper** from the **Microsoft Store**, choose your Star Citizen **LIVE** folder when asked (the Store app does not scan your drives), then paste your API key',
      '**macOS / Linux (or advanced Windows):** use **Python scripts** from Downloads (Python 3 + requirements.txt), then run `dumper.py --watch` — scripts may auto-detect installs',
      'Does **not** download your craft-bench / inventory from CIG — only blueprint award lines found in local log files you grant access to',
      'First run can import awards from `.log` files under your granted folder / detected log dirs, then watch mode tails the live Game.log',
      ...
      'Windows Store installs update through the Microsoft Store',
      'Older builds are blocked by the server — update via the Store or pull updated scripts if prompted',
