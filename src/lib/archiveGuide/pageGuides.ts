...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Scanner OCR** (signed-in members only — button in the calculator header): run **Dumper Apps** / BP Dumper in watch mode on the same PC, then click **OCR** in the Rock Calculator while seated in the **Mole pilot** chair after a **completed rock scan** — the **RESULTS** panel on the right must show scan data (mass, RES, INST, COMP SCU, composition %). Capture briefly switches to Star Citizen to grab your HUD, then returns to the Rock Calculator tab with live scan progress while your PC reads the panel. **First-time setup:** right-click the Dumper Apps tray icon → **Calibrate RESULTS panel** (draw the box once per resolution). If OCR fails, confirm Dumper Apps is still running in watch mode and the RESULTS panel still shows a finished scan',
      'After OCR fill the calculator stays in **scan mode** until you pick an ore from search or click a tracked card again — composition % come straight from your HUD (not spawn-location ranges); inert is still auto-calculated',
      ...
      '**CHP** (Crew Head Plan) beside OCR when **Crew** is checked — opens a Mole crew head-plan modal when your calculator has mass/resistance and a Mole loadout is selected. Same panel as Smart Cracker crew mode',
      '**SHP** (Solo Head Plan) beside OCR when **Crew** is unchecked — opens the solo garage · 3-head spread panel from Smart Cracker so you can see heavy / daily / finesse roles and any gaps in your Mole setup',
      '**Mole + Solo** (**Crew** unchecked in the calculator, or **Solo mining** checked in Smart Cracker): one laser at a time like a Prospector — per-head pro-tips appear on each hardpoint in Smart Cracker (collapsed by default on Mole; expand to read)',
  ...
  },
  {
    ...
    details: [
      ...
      '**First time only:** right-click the DR tray icon → **Calibrate RESULTS panel** (once per screen resolution)',
      'In-game: Mole pilot seat after a completed rock scan — RESULTS panel must show mass, RES, and composition → Mining Tracker → Rock Calculator → **OCR**',
      'Mac and Linux builds sync blueprints only — Rock Calculator OCR is Windows-only today',
