...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Scanner OCR** (button in the calculator header): ALT+PrtSc the in-game SCAN RESULTS panel, paste here with Ctrl+V, crop the scan panel (ore name, MASS, RES, INST, COMPOSITION, and composition % rows), and **Process** to auto-fill mass, resistance, instability, SCU, composition %, and Q bands. Primary ore is read from the ore label or both composition bands for that ore; duplicate lines are sorted High/Low by **smaller % = High**',
      'After you set the orange crop box, **zoom**, **tilt**, and **pan** once, those settings are **remembered** on your device — paste and Process without re-adjusting when your HUD layout stays the same. Scroll the preview to zoom; drag the image to pan when zoomed in',
      'After OCR fill the calculator stays in **scan mode** until you pick an ore from search or click a tracked card again — composition % come straight from your HUD (not spawn-location ranges); inert is still auto-calculated. The screenshot is never saved',
      ...
      'Use **Tilt** in the OCR window when the panel looks skewed or quality numbers are clipped at the edge of your crop box',
