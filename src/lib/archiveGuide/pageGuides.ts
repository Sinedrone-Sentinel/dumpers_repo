import { advisorLockPhraseHelpArchiveLines } from '../miningAdvisorLockPhraseHelp'
...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Crafting Wishlist** (signed in) sits after Can Craft. You can keep 10 named lists, and each list holds 20 unique recipes. The same blueprint at the same material qualities stacks and shows a quantity; a different quality mix is its own recipe',
      'On any blueprint detail, **Add to Crafting Wishlist** sits in Final Combined Modifiers (or under the materials when that panel is hidden). Pick one or more lists, set a quantity, and the list saves that blueprint with the qualities you had selected',
      'Open a Crafting Wishlist to rename it, delete it, change a recipe quantity, or remove a blueprint. **Got it** lowers that recipe by one. With **Use My Tracked Resources** checked, each material turns green when My Resources has enough of that exact quality and red when it does not, and Got it also deducts one craft from My Resources',
  ...
  },
  {
    ...
    details: [
      ...
      'Next to the ore search, type an **RS signature** from your scanner. The list only offers ores that can actually spawn that many rocks — a 4-rock reading is left out when the game never drops more than 3, and skipped cluster sizes (such as 2-rock Aluminum) are left out too. **Surface** and **Asteroid** are separate rows when both can produce that number. Picking a row fills the ore search and clears the signature box',
      '**Anomaly Override**, beside the signature box, is off unless you check it, and that choice is not saved. Use it only when you trust the scanner number and the list stayed empty. It then lists every ship ore whose base signature divides that number evenly, even when that many rocks is outside the usual cluster sizes. Gems never appear',
      ...
      'Signed-in members can open the **Advisor** chat in Smart Cracker for loadout ideas. It does not change your heads or Save. Paste your own free or paid Gemini access each visit, or **Save to my profile** with a lock phrase. If nothing is saved, you paste again next time. Clear a saved copy from Settings (the key is never shown). **Use scanned info** sends this rock’s mass, resistance, and instability; turn it off for ideas about the same resource without using that scan',
      ...advisorLockPhraseHelpArchiveLines(),
      'Advisor can also tell you **where to buy** a mining head, module, or gadget — ask something like "where can I buy the Hofstede-S2" and it lists every known shop with prices, grouped by system and cheapest first. Buy locations come from **UEX** and are crowdsourced, so treat prices as close rather than guaranteed. It only covers mining gear: it will not answer where to buy ammo, armour, food, or ships. For ore prices use **Commodity Lookup** instead',
      ...
      'In the loadout editor, the **Effective totals** card shows two numbers per stat: the normal value is your passive baseline, and a **blue value after the /** is that stat with all active modules switched on. Cards list every head/module stat from the game files — including **charge window rate** (how fast the green band fills), **cluster factor**, and **overcharge rate**. Power required still uses fracture power, resistance, and instability',
  ...
  },
  {
    ...
    details: [
      ...
      'On a WTS listing, check Deduct from Tracked Resources on commodity lines to pull that stock from Tracked Resources when you mark the sale ready (at the listed quality). Finished blueprint items are not deducted',
      ...
      'Each purchase or fulfillment claim becomes its own child transaction with the normal handoff, 72-hour deadlines, and ratings. Reputation progress counts each item on that deal (a quantity of two crafts counts as two toward the 5)',
      ...
      'On an accepted deal, Message opens a private chat with the other party (both sides need a verified RSI Handle). Chat closes and messages are removed when the deal completes, cancels, or times out',
      'Track active, completed, and archived transactions; confirm pickup on Active, then archive & rate on Completed',
      'After pickup, open the Completed tab — each order shows a purple Archive & rate button; both parties must rate (optional comment) to unlock new trades. After 5 items, click a visible star rating to read those comments',
  ...
  },
  {
    ...
    details: [
      ...
      'When fulfilling a WTB line, you can check Deduct from Tracked Resources to pull that line’s materials at the listed qualities when you mark the craft ready. Checkboxes disable in real time if remaining stock at those qualities is not enough',
      ...
      'Message on an accepted sale or fulfillment opens the same deal chat as My Listings — both parties must have a verified RSI Handle; messages are removed when the deal completes, cancels, or times out',
  ...
  },
  {
    ...
    details: [
      ...
      'My activity: pings when someone else moves your deal forward (accepted, ready, completed, released, timeout, no-show, deal chat, etc.) — requires verified RSI Handle',
  ...
  },
  {
    ...
    details: [
      ...
      'Alerts cover marketplace events such as order acceptances, ready-for-pickup, deal chat, and status changes',
      ...
      'Deal chat alerts stay until the deal ends — Open chat does not dismiss them, and Clear / Clear all skip them',
      ...
      'Use Clear all to dismiss other notifications at once (questionnaire prompts and open deal chats stay until they resolve)',
  ...
  },
  {
    ...
    details: [
      ...
      'Everyone has a Default group (always at the bottom) — new friends start there; you cannot rename or delete Default',
      ...
      'Friends can see and trade each other’s open Bazaar listings like any other member',
  ...
  },
  {
    ...
    details: [
      ...
      'First run can import awards from local logs, then watch mode tails the live Game.log',
      'Full history import (Enter = Y) scans every available log with no version filter; recent backup import only includes logs from the current game patch',
      ...
      'Store updates install from the Microsoft Store. For the exe, when a new Windows build is required, download DumperApps.exe from GitHub Releases and replace the old file yourself (no auto-download)',
      ...
      'Only one BP Dumper runs at a time — the Store app, **DumperApps.exe**, and `dumper.py` share one lock. A second copy exits and tells you to close the other window',
  ...
  },
  {
    id: 'help-bot',
    title: 'Help Chat',
    description: 'Ask how anything on this site works, from any page.',
    details: [
