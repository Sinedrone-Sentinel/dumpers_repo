...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Windows (recommended):** install **BP Dumper** from the **Microsoft Store**, open it, pick your LIVE folder, then paste your API key',
      '**Windows (portable exe):** download **DumperApps.exe** — auto-detects LIVE (or paste a path). This build is **unsigned**; Windows Defender / SmartScreen often block it (false positive) — prefer the Store when you can',
      'Published portable Windows builds only go live after the VirusTotal CI gate — named malware-family hits block publish; common generic/ML heuristic labels are ignored (you may still see them on the VirusTotal report)',
      ...
      'You can paste a custom path if auto-detect misses your install (portable / scripts)',
