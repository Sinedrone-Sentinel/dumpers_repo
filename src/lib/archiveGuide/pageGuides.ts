...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Windows:** download **DumperApps.exe** (native Go build), then point it at your Star Citizen **LIVE** folder (the folder that contains Game.log) and paste your API key',
      'Published Windows builds only go live after a clean VirusTotal scan in CI — open the VirusTotal report from Dumper Apps or the GitHub Release',
      '**macOS / Linux (or advanced Windows):** use **Python scripts** from Downloads (Python 3 + requirements.txt), then run `dumper.py --watch` with the same LIVE folder path',
      'You must choose the LIVE / log folder yourself — the app does not scan your drives for Star Citizen',
      'Does **not** download your craft-bench / inventory from CIG — only blueprint award lines found in local log files',
      'First run can import awards from `.log` files under your chosen folder (and logbackups), then watch mode tails the live Game.log',
