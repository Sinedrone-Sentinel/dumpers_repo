...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**BP Wishlist** (signed in) sits after Can Craft. You can keep 10 named lists, and each list holds 20 unique recipes. The same blueprint at the same material qualities stacks and shows a quantity; a different quality mix is its own recipe',
      'On any blueprint detail, **Add to Wishlist** sits in Final Combined Modifiers (or under the materials when that panel is hidden). Pick one or more lists, set a quantity, and the list saves that blueprint with the qualities you had selected',
      'Open a wishlist to rename it, delete it, change a recipe quantity, or remove a blueprint. **Got it** lowers that recipe by one. With **Use My Tracked Resources** checked, each material turns green when My Resources has enough of that exact quality and red when it does not, and Got it also deducts one craft from My Resources',
