...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Scanner OCR** (signed-in members only — button in the calculator header): run **BP Dumper** in watch mode on the same PC (starts the tray + scan bridge), then click **OCR** while the Mole pilot **RESULTS** panel is open in-game. Desktop capture switches to Star Citizen and fills mass, RES, INST, COMP SCU, composition %, and Q bands. **First-time setup:** right-click the BP Dumper tray icon → **Calibrate RESULTS panel** (draw the box once per resolution). If OCR fails, confirm the DR tray icon is running on this PC',
