...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Scanner OCR** (signed-in members only — button in the calculator header): run **BP Dumper** in watch mode on the same PC (starts the tray + scan bridge), then click **OCR** while the Mole pilot **RESULTS** panel is open in-game. Desktop capture auto-fills mass, RES, INST, COMP SCU, composition %, and Q bands. **First-time setup:** right-click the BP Dumper tray icon → **Calibrate RESULTS panel** (draw the box once per resolution)',
      'If the desktop bridge is offline, signed-in members can still paste an ALT+PrtSc screenshot into the OCR modal as a fallback — crop the full RESULTS block (header through composition rows) and **Process**',
