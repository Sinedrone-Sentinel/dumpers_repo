...
export const PAGE_GUIDES: ArchivePageGuide[] = [
  ...
  },
  {
    ...
    details: [
      ...
      '**Windows (recommended):** install **BP Dumper** from the **Microsoft Store**, open it, then paste your API key when prompted',
      '**Windows (optional):** **Windows portable exe** if you prefer not using the Store',
      '**macOS / Linux:** use **Python scripts** from Downloads (Python 3 + requirements.txt), then run `dumper.py --watch`',
      ...
      'Store installs update through the Microsoft Store; on the portable exe, **Keep App Up to Date** defaults to Yes and can download a newer exe from GitHub',
      'Older builds are blocked by the server — update via the Store, get the latest portable exe, or pull updated scripts if prompted',
