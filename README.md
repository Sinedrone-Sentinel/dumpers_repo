...
The production build regenerates [`public/archive-guide.html`](public/archive-guide.html) for Save-as-PDF offline reading (`npm run generate-archive-guide`), and bakes the same content into `supabase/functions/site-help-bot/knowledge.json` for the Help chat.

### Help chat

- **?** button at the right end of the header, on every page for signed-in members
- Answers how-to questions about the site from the Information Archive only — it refuses game-data questions and cannot act on the member's behalf
