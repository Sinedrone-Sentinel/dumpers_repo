...
- Answers how-to questions from the Information Archive, and catalog questions (Wikelo trades, missions, blueprints, components, and the other reference lists) from the baked site data. It does not invent prices or loadouts, and it cannot act on the member's behalf
- Runs on the member's own Gemini key, sharing the encrypted saved key with the Smart Cracker Advisor
- Its own 20 asks/hour bucket (migration 195), with a live `x/20 this hour` meter in the chat header that updates on each ask and polls every two minutes
