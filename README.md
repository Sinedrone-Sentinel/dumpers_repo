...
- **RS signature** field next to the ore search matches a scanner reading only to cluster sizes the game files actually spawn (skipped sizes such as 2-rock Aluminum are excluded). Surface and Asteroid are separate hits when both can produce that reading; choosing one fills the ore search and clears the signature box
- **Anomaly Override** (unchecked, not saved) lists every ship ore whose base signature divides the typed number evenly, including cluster sizes the files never spawn. Gems are never offered — their rock entities share a placeholder signature, not a ship scanner value
...
- **My open listings** panel: edit line quantities, remove lines, or close a listing inline; WTS commodity lines can optionally deduct from Tracked Resources
...
The production build regenerates [`public/archive-guide.html`](public/archive-guide.html) for Save-as-PDF offline reading (`npm run generate-archive-guide`), and bakes that guide plus the Wikelo, mission, blueprint, and other site catalogs into `supabase/functions/site-help-bot/knowledge.json` for the Help chat.

### Help chat

- **?** button at the right end of the header, on every page for signed-in members
- Answers how-to questions from the Information Archive, and catalog questions from the baked lists that question needs (missions and factions for a contract question, Wikelo only when the question is about Wikelo). It does not invent prices or loadouts, and it cannot act on the member's behalf
- Runs on the member's own Gemini key, sharing the encrypted saved key with the Smart Cracker Advisor
- Its own 20 asks/hour bucket (migration 195). The `x/20 this hour` meter moves only when the question is sent to Gemini. Migration 200 gives the ask back if that request never goes out
- Edge Function `site-help-bot`; knowledge base regenerated by `npm run build-help-knowledge-base` (chained into `generate-archive-guide`)
...
**RSI Handle verification** is required to post listings or trade on the Bazaar, register personal Discord webhooks, and use Mining Ledgers. Members verify with **Link Citizen iD** in Settings or welcome onboarding (Edge `link-citizenid` / `citizenid-oauth-callback`). Removing the Citizen iD link un-verifies the member.
...
| `npm run generate-archive-guide` | Regenerate `public/archive-guide.html` and the Help chat knowledge base |
| `npm run build-help-knowledge-base` | Rebake `supabase/functions/site-help-bot/knowledge.json` from the Archive |
