...
Same `.env` keys as `DumperApps.exe` and `dumper.py`. Drop `BPDumperGUI.exe` next to an existing `.env` or use **File → Load**.

This is **not** the member GitHub download and **not** a Store package. Member Windows download stays `DumperApps.exe`.

## Rights model

- You pick the Star Citizen LIVE folder (**File → Path**). No drive scan.
...
2. **File → Load** an existing `.env`, or type the key and set options.
3. **File → Path** — choose the LIVE folder that contains `Game.log`.
4. **File → Save** writes `.env` beside the exe (or the file you loaded).
